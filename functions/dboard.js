...
const getMoreComments = require('./helpers/getMoreComments');
const getMoreTopics = require('./helpers/getMoreTopics');
require('dotenv').config();

exports.handler = async (event,context,callback) => {
    if (event.httpMethod === 'GET' && event.path == '/api/dboard/topics') {
        return await getTopics(event);
    } else if (event.httpMethod === 'POST' && event.path == '/api/dboard/topics') {
        return await getMoreTopics(event);
    ...
    } else if (event.httpMethod === 'POST' && event.path.indexOf('/api/dboard/comments/')>-1) {
        return await getMoreComments(event);
